...
@pytest.mark.asyncio
async def test_configure_sync_add_remove_list(mock_bot, mock_interaction, temp_config_file):
    ...
    with patch("src.commands.configure_sync.CONFIG_PATH", temp_config_file):
        ...
        # Add a tournament
        await cog.add_tournament.callback(cog, mock_interaction, slug="LCS_2024_Summer")
        mock_interaction.response.send_message.assert_called_with(
            "Successfully added `LCS_2024_Summer` to the sync list.", ephemeral=True
        )

        # List tournaments
        await cog.list_tournaments.callback(cog, mock_interaction)
        assert "LCS_2024_Summer" in mock_interaction.response.send_message.call_args[1]["embed"].description

        # Remove tournament
        await cog.remove_tournament.callback(cog, mock_interaction, slug="LCS_2024_Summer")
        mock_interaction.response.send_message.assert_called_with(
            "Successfully removed `LCS_2024_Summer` from the sync list.", ephemeral=True
        )

@pytest.mark.asyncio
@patch("src.commands.sync_leaguepedia.LeaguepediaClient")
async def test_sync_leaguepedia_command(MockLeaguepediaClient, mock_bot, mock_interaction, temp_config_file):
    """Tests the main sync-leaguepedia command."""
    # Setup mock client
    mock_client_instance = MockLeaguepediaClient.return_value
    mock_client_instance.get_tournament_by_slug = AsyncMock(return_value={"Name": "LCS", "DateStart": "2024-01-01", "DateEnd": "2024-12-31"})
    mock_client_instance.get_matches_for_tournament = AsyncMock(return_value=[{"MatchId": "1", "Team1": "Team A", "Team2": "Team B", "DateTime_UTC": "2024-05-05T12:00:00Z"}])
    ...
    with patch("src.commands.sync_leaguepedia.CONFIG_PATH", temp_config_file), \
         patch("src.commands.sync_leaguepedia.get_async_session", get_test_async_session):
