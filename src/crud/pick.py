...
def list_picks_for_match_with_users(
    session: Session, match_id: int
) -> List[Pick]:
    """
    List picks for a match, eagerly loading the associated user.
    """
    logger.debug("Listing picks (with users) for match ID: %s", match_id)
    statement = (
        select(Pick)
