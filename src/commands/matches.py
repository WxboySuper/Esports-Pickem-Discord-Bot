...
async def contest_autocompletion(
    ...
    async with get_async_session() as session:
        all_contests = await crud.list_contests(session)
...
class DayNavigationView(discord.ui.View):
    ...
    async def update_embed(self, interaction: discord.Interaction):
        ...
        async with get_async_session() as session:
            matches = await crud.get_matches_by_date(
...
class TournamentSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        async with get_async_session() as session:
            matches = await crud.list_matches_for_contest(session, contest_id)
...
async def view_by_day(interaction: discord.Interaction):
    ...
    async with get_async_session() as session:
        matches = await crud.get_matches_by_date(session, current_date)
...
async def upload(
    ...
    async with get_async_session() as session:
        ...
        try:
            ...
            await crud.bulk_create_matches(session, matches_to_create)
            await interaction.followup.send(
