...
from src.models import Contest, Match, Team
...
class SyncLeaguepedia(commands.Cog):
    ...
    @app_commands.command(name="sync-leaguepedia", description="Syncs all configured tournaments from Leaguepedia.")
    @app_commands.check(is_admin)
    async def sync_leaguepedia(self, interaction: discord.Interaction):
        """
        Performs a full sync of tournaments, matches, and teams from Leaguepedia
        based on the configured tournament slugs.
        """
        await interaction.response.defer(ephemeral=True, thinking=True)

        if not CONFIG_PATH.exists():
            await interaction.followup.send("Configuration file not found. Please add tournaments first using `/configure-sync add`.")
            return
        ...
        if not tournament_slugs:
            await interaction.followup.send("No tournaments configured for sync. Please add some using `/configure-sync add`.")
            return
        ...
        async with aiohttp.ClientSession() as http_session:
            ...
            async with get_async_session() as db_session:
                for slug in tournament_slugs:
                    ...
                    if not contest_data:
                        await interaction.followup.send(f"Could not find tournament: `{slug}`. Skipping.", ephemeral=True)
                        continue
                    ...
                    contest = await upsert_contest(db_session, {
                        "leaguepedia_id": slug,
                    ...
                    for match_data in matches_data:
                        ...
                        for team_name_key in ["Team1", "Team2"]:
                            ...
                            if team_api_data:
                                await upsert_team(db_session, {
                                    "leaguepedia_id": team_name, # Assuming name is unique ID
