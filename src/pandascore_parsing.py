...
def _parse_pandascore_date(date_str: Optional[str]) -> Optional[datetime]:
    if not date_str:
        return None
    try:
...
def _extract_team_data(opponent: Dict[str, Any]) -> Optional[Dict[str, Any]]:
...
def _extract_contest_data(match_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    scheduled_at = _parse_pandascore_date(match_data.get("scheduled_at"))
...
def _extract_winner_and_scores(
